...
\begin{abstract}
    \noindent
    This paper presents a genetic algorithm approach to solve the Traveling Salesman Problem (TSP). The implementation focuses on key aspects of genetic algorithms, including problem representation, parent selection methods (tournament, roulette, rank-based, and random), crossover techniques (POS and OX1), and mutation operators (exchange, insertion, and IVM). The algorithm was evaluated on a standard TSP dataset (bays29.tsp) using a grid search to optimize hyperparameters. The results show the effectiveness of the genetic algorithm in finding near-optimal solutions to the TSP, with a best solution within 1.73\% of the optimal known solution. The analysis of hyperparameters reveals the importance of elitism, number of generations and individuals and the robustness of the algorithm to different crossover and mutation techniques, as well as the great performance of tournament selection. 
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Analyze} the impact of different parameter configurations on the solution quality.
\end{enumerate}
...
The best configuration identified during the grid search yielded a solution within 1.73\% of the optimal known solution for the bays29 dataset. This shows the effectiveness of GAs in tackling the TSP, even for relatively small problem instances.

The analysis of hyperparameters highlighted the importance of elitism in preserving high-quality solutions and the robustness of the algorithm to different crossover and mutation techniques. While the results for different crossover and mutation operators were similar, further exploration with a larger dataset and more complex problem instances could reveal potential differences in their performance. Besides this, we noticed the great performance of tournament selection as a parent selection method. In addition to this, the amount of generations and population size were proven to have a great influence in the results, as expected.
