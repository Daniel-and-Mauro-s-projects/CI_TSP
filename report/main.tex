...
In order to evaluate the influence of each hyperparameter on the performance of the genetic algorithm, we conducted a grid search over a range of values for each parameter. The hyperparameters considered in the grid search were:
\begin{itemize}
...
\end{itemize}
In total we considered 3240 different configurations, and we ran the genetic algorithm for each configuration to evaluate its performance.
\subsection{Best Configuration}

\begin{wrapfigure}{r}{0.55\textwidth}
    \centering
    \includegraphics[width=0.6\textwidth]{../results/best_chromosme.png}
    \caption{Best route found by the genetic algorithm.}
